...
const Hero3DElement = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (event: MouseEvent) => {
      ...
      // Increased sensitivity for more noticeable movement, adjust as needed
      const xRotation = (clientY / innerHeight - 0.5) * 6; 
      const yRotation = (clientX / innerWidth - 0.5) * -6; 
      // Apply transform with smoother transition (already handled by CSS on .hero-3d-element)
  ...
  return (
    <div ref={elementRef} className="hero-3d-element pointer-events-none opacity-40 group-hover:opacity-60 transition-opacity duration-500">
...
export default function Home() {
    ...
    if (storedHistory) {
      try {
        ...
        if (Array.isArray(parsedHistory)) {
            ...
            setHistoryItems([]); // Initialize as empty if not an array
        }
      } catch (error) {
        console.error("Failed to parse history from localStorage:", error);
        setHistoryItems([]); // Initialize as empty on error
      }
    }
  }, []);
  ...
  const handleResult = (newResult: SimplificationResult | null, text?: string, lang?: string) => {
    setIsLoading(newResult === null && !!text); // Set loading only if text is provided and newResult is null (initial request)
    setResult(newResult);
  ...
  return (
    <div className="flex flex-col min-h-screen items-center relative overflow-x-hidden bg-background text-foreground">
      ...
      <header className="w-full py-4 px-6 md:px-10 fixed top-0 z-50 bg-background/80 backdrop-blur-md shadow-lg">
        <div className="container mx-auto flex items-center justify-between">
          ...
          <nav className="hidden md:flex gap-2 items-center"> {/* Reduced gap for closer items */}
            {navItems.map((item) => (
              <button
                key={item.id}
                onClick={() => scrollToSection(item.id)}
                className="text-sm font-medium text-muted-foreground hover:text-primary transition-colors duration-200 capitalize px-3 py-2 rounded-md hover:bg-primary/10 futuristic-glow" // Added futuristic-glow for subtle effect
              >
                {item.label}
              </button>
            ))}
            <Sheet open={isHistoryPanelOpen} onOpenChange={setIsHistoryPanelOpen}>
              <SheetTrigger asChild>
                 <Button variant="outline" className="text-primary hover:text-primary-foreground hover:bg-primary futuristic-glow-primary"> {/* Standard primary glow */}
                   <HistoryLucideIcon className="mr-2 h-4 w-4" /> History
                 </Button>
              </SheetTrigger>
              <SheetContent side="right" className="w-full sm:max-w-md md:max-w-lg bg-card/95 backdrop-blur-lg border-border/50 text-card-foreground p-0">
                <HistoryPanel
            ...
            <Button variant="default" onClick={() => scrollToSection('simplify')} className="futuristic-glow-accent">Get Started</Button> {/* Standard accent glow */}
          </nav>
          <div className="md:hidden flex items-center gap-2">
            <Sheet open={isHistoryPanelOpen} onOpenChange={setIsHistoryPanelOpen}>
              <SheetTrigger asChild>
                 <Button variant="ghost" size="icon" aria-label="View History" className="text-foreground hover:text-primary">
            ...
            <Button variant="ghost" size="icon" onClick={() => setIsMenuOpen(!isMenuOpen)} aria-label="Open Menu" className="text-foreground hover:text-primary">
      ...
      <section id="hero" className="w-full h-screen flex flex-col items-center justify-center relative text-center px-4 md:px-6 pt-20 overflow-hidden group">
        ...
        <Image 
            ...
            layout="fill"
            objectFit="cover"
            className="absolute inset-0 z-0 opacity-25" /* Reduced opacity */
        ...
        <div className="z-10 relative scroll-animate" style={{ animationDelay: '0.2s' }}>
          ...
          <p className="mt-6 md:mt-8 text-lg md:text-xl text-muted-foreground max-w-2xl mx-auto scroll-animate" style={{ animationDelay: '0.4s' }}>
          ...
          <div className="mt-10 md:mt-12 flex flex-col sm:flex-row gap-4 justify-center scroll-animate" style={{ animationDelay: '0.6s' }}>
            <Button size="lg" onClick={() => scrollToSection('simplify')} className="futuristic-glow-primary text-lg px-8 py-6 group/maincta bg-primary text-primary-foreground hover:bg-primary/90 transform hover:scale-105 transition-all duration-300">
              Translate and simplify any complex text <ArrowRight className="ml-2 h-5 w-5 group-hover/maincta:translate-x-1.5 transition-transform duration-300"/>
            </Button>
            <Button size="lg" variant="outline" onClick={() => scrollToSection('about')} className="text-lg px-8 py-6 border-accent text-accent hover:bg-accent/10 hover:text-accent-foreground futuristic-glow-accent transform hover:scale-105 transition-all duration-300">
              Learn More <ChevronDown className="ml-2 h-5 w-5 group-hover:rotate-[-5deg] transition-transform duration-300"/>
            </Button>
          </div>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-8 md:gap-12 items-center">
            <div className="scroll-animate" style={{ animationDelay: '0.2s' }}>
              <Image 
                ...
                width={800} 
                height={534} 
                className="rounded-lg shadow-2xl object-cover w-full transform hover:scale-105 transition-transform duration-500 ease-out futuristic-glow"
                data-ai-hint="businessman simplify"
              />
            </div>
            <div className="space-y-6 scroll-animate" style={{ animationDelay: '0.4s' }}>
              <h3 className="text-2xl font-semibold text-primary text-glow-primary">Our Mission</h3>
              ...
              <h3 className="text-2xl font-semibold text-primary text-glow-primary">Core Values</h3>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-2 gap-6 md:gap-8">
            {services.map((service, index) => (
              <button 
                ...
                className="text-left w-full h-full block focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 focus:ring-offset-background rounded-lg group/servicecard scroll-animate"
                style={{ animationDelay: `${index * 0.15}s` }}
                aria-label={`Learn more about ${service.title}`}
              >
                <Card className={`bg-card/60 backdrop-blur-sm shadow-xl group-hover/servicecard:shadow-2xl transition-all duration-300 transform group-hover/servicecard:scale-[1.04] h-full flex flex-col ${service.glowClass} futuristic-glow`}>
                  <CardHeader className="items-center text-center pt-6 pb-4">
                    <div className={`p-4 rounded-full bg-current/10 mb-4 transition-transform duration-300 group-hover/servicecard:scale-110 ${service.glowClass}`}>{service.icon}</div>
                    <CardTitle className={`text-xl font-semibold ${service.title.includes("Arabic") ? 'text-glow-accent' : (service.title.includes("Universal") ? 'text-glow-secondary' : 'text-glow-primary')}`}>{service.title}</CardTitle>
                  </CardHeader>
      ...
      <section id="simplify" className="w-full py-16 md:py-24 bg-background/50">
        <div className="container mx-auto px-4 md:px-6">
            ...
            <div className="w-full max-w-4xl mx-auto flex flex-col items-center space-y-10 scroll-animate" style={{ animationDelay: '0.2s' }}>
                <SimplificationForm 
      ...
      <section id="contact" className="w-full py-16 md:py-24 bg-background/50">
        <div className="container mx-auto px-4 md:px-6">
          ...
          <Card className="max-w-2xl mx-auto bg-card/60 backdrop-blur-sm border-border/40 shadow-xl p-6 md:p-8 scroll-animate futuristic-glow-primary transform hover:scale-[1.02] transition-transform duration-300" style={{ animationDelay: '0.2s' }}>
            <form className="space-y-6">
              <div className="grid sm:grid-cols-2 gap-6">
                <div className="scroll-animate" style={{ animationDelay: '0.3s' }}>
                  ...
                  <input type="text" id="name" name="name" className="w-full bg-input border-border/70 focus:border-primary focus:ring-2 focus:ring-primary/50 transition-all duration-200 ease-in-out shadow-inner rounded-md p-3 placeholder:text-muted-foreground/60 futuristic-glow-primary focus:shadow-md" placeholder="Your Name" />
                </div>
                <div className="scroll-animate" style={{ animationDelay: '0.4s' }}>
                  ...
                  <input type="email" id="email" name="email" className="w-full bg-input border-border/70 focus:border-primary focus:ring-2 focus:ring-primary/50 transition-all duration-200 ease-in-out shadow-inner rounded-md p-3 placeholder:text-muted-foreground/60 futuristic-glow-primary focus:shadow-md" placeholder="you@example.com" />
                </div>
              </div>
              <div className="scroll-animate" style={{ animationDelay: '0.5s' }}>
                ...
                <textarea id="message" name="message" rows={4} className="w-full bg-input border-border/70 focus:border-primary focus:ring-2 focus:ring-primary/50 transition-all duration-200 ease-in-out shadow-inner rounded-md p-3 placeholder:text-muted-foreground/60 min-h-[120px] futuristic-glow-primary focus:shadow-md" placeholder="Your message..."></textarea>
              </div>
              <div className="scroll-animate" style={{ animationDelay: '0.6s' }}>
                <Button type="submit" className="w-full text-lg py-3 bg-primary text-primary-foreground hover:bg-primary/90 transition-all duration-200 ease-in-out transform hover:scale-[1.02] focus:ring-4 focus:ring-primary/40 shadow-lg futuristic-glow-primary active:shadow-sm active:scale-[1.00]">
                  Send Message
                </Button>
              </div>
            </form>
      ...
      <footer className="w-full py-12 px-6 md:px-10 text-center z-10 border-t border-border/20 bg-background/70 backdrop-blur-sm">
        <div className="container mx-auto">
            <div className="flex justify-center gap-6 mb-6">
                <Link href="#" aria-label="Facebook" className="text-muted-foreground hover:text-primary transition-all duration-300 transform hover:scale-125 hover:text-glow-primary"><Facebook className="h-6 w-6"/></Link>
                <Link href="#" aria-label="Twitter" className="text-muted-foreground hover:text-primary transition-all duration-300 transform hover:scale-125 hover:text-glow-primary"><Twitter className="h-6 w-6"/></Link>
                <Link href="#" aria-label="LinkedIn" className="text-muted-foreground hover:text-primary transition-all duration-300 transform hover:scale-125 hover:text-glow-primary"><Linkedin className="h-6 w-6"/></Link>
                <Link href="#" aria-label="GitHub" className="text-muted-foreground hover:text-primary transition-all duration-300 transform hover:scale-125 hover:text-glow-primary"><Github className="h-6 w-6"/></Link>
            </div>
            <p className="text-sm text-muted-foreground scroll-animate">
              Powered by Genkit &amp; Gemini AI &bull; Crafted with Next.js &amp; ShadCN UI &bull; Design for Clarity
            </p>
            <p className="text-xs text-muted-foreground/70 mt-3 scroll-animate" style={{ animationDelay: '0.2s' }}>
